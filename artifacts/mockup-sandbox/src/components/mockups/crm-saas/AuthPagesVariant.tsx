...
function Logo() {
  return (
    <div className="flex items-center gap-2.5">
      <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#ffb45c] text-[#241d2a] shadow-[0_7px_18px_rgba(255,180,92,.24)]">
        <Zap size={18} fill="currentColor" strokeWidth={2.5} />
      </div>
      <div className="font-semibold tracking-[-.04em] text-[#fff7ea]">
        Connectly <span className="text-[#ffb45c]">CRM</span>
      </div>
    </div>
  );
}
...
function Rail() {
  return (
    <aside className="relative hidden min-h-[760px] w-[42%] shrink-0 overflow-hidden bg-[#292238] px-10 py-10 text-[#fff7ea] lg:flex lg:flex-col xl:px-14">
      <div className="absolute -right-24 top-20 h-72 w-72 rounded-full border border-[#ffb45c]/20" />
      <div className="absolute -right-10 top-34 h-52 w-52 rounded-full border border-[#ffb45c]/15" />
      <div className="absolute bottom-[-120px] left-[-80px] h-80 w-80 rounded-full bg-[#b35d62]/20 blur-3xl" />
      <div className="relative z-10">
        <Logo />
        <div className="mt-20 max-w-[390px]">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#ffb45c]/25 bg-[#ffb45c]/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[.15em] text-[#ffd49d]">
          ...
          <h1 className="font-display text-[clamp(38px,4vw,60px)] font-bold leading-[.98] tracking-[-.065em]">
            Start with the<br /><span className="text-[#ffb45c]">conversation.</span>
          </h1>
          ...
          <div className="mt-10 space-y-4">
            {benefits.map((benefit, index) => (
              <div key={benefit} className="flex items-start gap-3 text-[13px] leading-5 text-[#e6dfea]">
                <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#ffb45c] text-[#292238]">
                  <Check size={12} strokeWidth={3} />
                </span>
                <span><b className="mr-1 text-[#ffcf91]">0{index + 1}</b> {benefit}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="relative z-10 mt-auto flex items-end justify-between border-t border-white/10 pt-5">
        ...
        <div className="flex -space-x-2">
          {["NA", "JL", "PK"].map((initials, index) => (
            <div key={initials} className={`flex h-8 w-8 items-center justify-center rounded-full border-2 border-[#292238] text-[9px] font-bold ${["bg-[#f2c6ad] text-[#713b2a]", "bg-[#b9d6ee] text-[#244b71]", "bg-[#d7c1e8] text-[#593d6f]"][index]}`}>{initials}</div>
          ))}
        </div>
      </div>
    </aside>
  ...
  return (
    <label className="block">
      ...
      <span className="relative block">
        ...
        <input type={type} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} required className="h-12 w-full rounded-xl border border-[#ded5da] bg-[#fffdf9] pl-11 pr-10 text-[13px] text-[#302735] outline-none transition focus:border-[#d7844c] focus:ring-4 focus:ring-[#d7844c]/10 placeholder:text-[#ada5ac]" />
      </span>
    </label>
  );
}
...
function SignupFields({ step, setStep }: { step: SignupStep; setStep: (step: SignupStep) => void }) {
  ...
  if (step === 1) return (
    <div className="space-y-5">
      ...
      <button onClick={() => setStep(2)} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#d7844c] text-[13px] font-bold text-[#fffaf2] transition hover:bg-[#c87540]">Create my workspace <ArrowRight size={16} /></button>
  ...
  if (step === 2) return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{["Just me", "2–5", "6–20", "21+"].map((size) => <button key={size} onClick={() => setTeam(size)} className={`h-11 rounded-xl border text-[12px] font-bold transition ${team === size ? "border-[#d7844c] bg-[#fff0e3] text-[#af5c2e]" : "border-[#ded5da] text-[#776c78] hover:border-[#c4b8bf]"}`}>{size}</button>)}</div>
      </div>
      <button onClick={() => setStep(3)} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#d7844c] text-[13px] font-bold text-[#fffaf2] transition hover:bg-[#c87540]">Continue <ArrowRight size={16} /></button>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-[#eed7c7] bg-[#fff3e8] p-4 text-[12px] leading-5 text-[#795643]"><b className="text-[#a8522d]">One last step.</b> We&apos;ll send a test message after you connect Meta, so your team can see the flow before going live.</div>
      <button onClick={() => setStep(1)} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#d7844c] text-[13px] font-bold text-[#fffaf2] transition hover:bg-[#c87540]">Finish setup <Check size={16} /></button>
...
export function AuthPagesVariant() {
  ...
  return (
    <main className="flex min-h-[100dvh] w-full bg-[#f7f1ea] font-sans">
      <Rail />
      <section className="flex min-w-0 flex-1 flex-col px-5 py-6 sm:px-10 lg:px-16 xl:px-24">
        ...
        <div className="mx-auto flex w-full max-w-[470px] flex-1 flex-col justify-center py-10">
          <div className="mb-9 flex rounded-xl border border-[#dfd4d5] bg-[#fdf9f4] p-1">
            <button onClick={() => { setMode("login"); setNotice(""); }} className={`h-10 flex-1 rounded-lg text-[12px] font-bold transition ${mode === "login" ? "bg-[#292238] text-[#fff7ea] shadow-sm" : "text-[#827682]"}`}>Sign in</button>
            <button onClick={() => { setMode("signup"); setNotice(""); }} className={`h-10 flex-1 rounded-lg text-[12px] font-bold transition ${mode === "signup" ? "bg-[#292238] text-[#fff7ea] shadow-sm" : "text-[#827682]"}`}>Create account</button>
          </div>
          <div className="mb-8">
            <p className="mb-3 text-[11px] font-bold uppercase tracking-[.14em] text-[#b2633d]">{mode === "login" ? "Welcome back" : `Getting started · 0${step} of 03`}</p>
          ...
          {mode === "login" ? (
            <form onSubmit={submit} className="space-y-5">
              <button type="button" onClick={() => setNotice("Google sign-in is ready for your workspace.")} className="flex h-12 w-full items-center justify-center gap-3 rounded-xl border border-[#ded5da] bg-[#fffdf9] text-[13px] font-bold text-[#514653] transition hover:border-[#c4b8bf]"><span className="font-display text-[18px] font-bold text-[#db7145]">G</span> Continue with Google</button>
              ...
              <label className="block"><div className="mb-2 flex items-center justify-between"><span className="text-[11px] font-bold uppercase tracking-[.1em] text-[#756b79]">Password</span><button type="button" onClick={() => setNotice("Password reset link requested.")} className="text-[11px] font-bold text-[#b45d37]">Forgot password?</button></div><span className="relative block"><LockKeyhole size={16} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[#a49aa7]" /><input type={showPassword ? "text" : "password"} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter your password" required className="h-12 w-full rounded-xl border border-[#ded5da] bg-[#fffdf9] pl-11 pr-11 text-[13px] outline-none focus:border-[#d7844c] focus:ring-4 focus:ring-[#d7844c]/10" /><button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#9c929b]">{showPassword ? <EyeOff size={16} /> : <Eye size={16} />}</button></span></label>
              <button type="submit" className="h-12 w-full rounded-xl bg-[#d7844c] text-[13px] font-bold text-[#fffaf2] transition hover:bg-[#c87540]">Sign in to Connectly <ArrowRight className="ml-1 inline" size={15} /></button>
            </form>
          ...
          <p className="mt-9 text-center text-[12px] text-[#948b93]">{mode === "login" ? "New to Connectly?" : "Already have an account?"} <button onClick={() => setMode(mode === "login" ? "signup" : "login")} className="font-bold text-[#b45d37] hover:underline">{mode === "login" ? "Start a free workspace" : "Sign in instead"}</button></p>
        </div>
